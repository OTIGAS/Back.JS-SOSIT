  ...
  async listarTodosClientes() {
    try {
      return await new Promise((resolve, reject) => {
        this.db.query(
          `
            SELECT u.id, u.nome, u.email as email_login, u.tipo, c.*
            FROM usuario u
            INNER JOIN contato c ON u.id_contato = c.id
  ...
  async listarTodasEmpresas() {
    try {
      return await new Promise((resolve, reject) => {
        this.db.query(
          `
            SELECT u.id, u.nome, u.email, u.tipo, c.*, e.*, ie.*, db.* FROM usuario u
            INNER JOIN contato c ON u.id_contato = c.id
            INNER JOIN endereco e ON u.id_endereco = e.id
            INNER JOIN informacoes_empresa ie ON u.id_informacoes_empresa = ie.id
            INNER JOIN dados_bancarios db ON u.id_dados_bancarios = db.id
  ...
  async listarClientePorId(idUsuario) {
    try {
      return await new Promise((resolve, reject) => {
        this.db.query(
          `
            SELECT u.id, u.nome, u.email, u.tipo, c.* FROM usuario u
